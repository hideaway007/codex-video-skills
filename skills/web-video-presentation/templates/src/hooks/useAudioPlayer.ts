...
interface Options {
  ...
  mode: PlaybackMode;
  /** Tiny HTML handoff buffer (ms) after audio finishes before advancing,
   *  in `auto` mode. This lets the browser flush the final audio samples
   *  without adding a visible sentence-to-sentence wait. */
  trailMs?: number;
 ...
 *
 * In `auto` mode:
 *   • Audio file present → advance shortly after the audio's `ended` event.
