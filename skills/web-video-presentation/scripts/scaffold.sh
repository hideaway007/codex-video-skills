...
DEFAULT_THEME="d2-editorial-grid"
...
echo "▸ 使用主题：$THEME"
(cd "$TARGET_PARENT_ABS" && npm create vite@latest "$TARGET_NAME" -- --template react-ts >/dev/null)

cd "$TARGET_ABS"
echo "▸ 安装依赖（可能要等一会）..."
npm install >/dev/null 2>&1

echo "▸ 安装脚本依赖（tsx + puppeteer-core）..."
npm install --save-dev tsx puppeteer-core >/dev/null 2>&1
...
cp "$TEMPLATES/src/hooks/useMasterAudioTimeline.ts" src/hooks/useMasterAudioTimeline.ts
...
# Audio + recording pipeline scripts.
...
cp "$TEMPLATES/scripts/normalize-audio.mjs"        scripts/normalize-audio.mjs
cp "$TEMPLATES/scripts/build-master-audio.mjs"     scripts/build-master-audio.mjs
cp "$TEMPLATES/scripts/record-cdp-screencast.mjs"  scripts/record-cdp-screencast.mjs
cp "$TEMPLATES/scripts/burn-subtitles.mjs"         scripts/burn-subtitles.mjs
...
p.scripts = Object.assign({}, p.scripts, {
  ...
  "normalize-audio":    "node scripts/normalize-audio.mjs",
  "build-master-audio": "node scripts/build-master-audio.mjs",
  "record-cdp":         "node scripts/record-cdp-screencast.mjs",
  "burn-subtitles":     "node scripts/burn-subtitles.mjs",
});
...
音频合成（可选，录制前做）：
  ...
  npm run synthesize-audio      # 默认火山/豆包：刘飞 2.0 → public/audio/<id>/<step>.mp3
                                # 需要 DOUBAO_TTS_API_KEY 或 VOLCENGINE_TTS_API_KEY
                                # （本地 MeloTTS 可传 -- --engine=melotts；MiniMax 可传 -- --engine=mmx）
...
写章节时必读（单一入口，路径在 SKILL 仓库内）：
  ...
  • ${SKILL_DIR}/references/RICH-HTML-TOOLKIT.md
      需要数字、流程、对比、状态、dashboard 或更丰富 React/SVG 组件时翻：
      metric card / SVG chart / gauge / flow path / browser mock / terminal mock
